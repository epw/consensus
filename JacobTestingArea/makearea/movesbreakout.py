...
def movesdata():
	...
	for move in moveslist:		movesthing.append({'name':'','before':'', 'trigger':'', 'after':'', 'movestring':'', 'afterlist':'', 'description':'', 'list':[], 'number':'', 'secondlist':[], 'secondafter':''})
		...
		if len(move) == 2:
			...
			list = move[0].split('**')
			movedict['movestring'] = movedict['before'] + movedict['trigger'] + movedict['after'] + str(movedict['list']) + movedict['afterlist']	output = {'assert':{}, 'impress':{}, 'sleeper':{}, 'help':{}, 'sellalie':{}, 'outfox':{}, 'headdown':{}, 'rending':{}, 'aligned':{}, 'suddenchange':{}, 'backlash':{}}	for key in output.keys():				for x in movesthing:			if key in x['name']:				output.update({key:x})	return outputif __name__ == "__main__":
	get_data()	dict = movesdata()	template = latex_jinja_env.get_template('basicmoves-template.tex')	output = template.render(Assert = dict['assert'], impress = dict['impress'], sleeper = dict['sleeper'], help = dict['help'], sell = dict['sellalie'], outfox = dict['outfox'], headdown = dict['headdown'], rending = dict['rending'], aligned = dict['aligned'], suddenchange = dict['suddenchange'], backlash = dict['backlash'])	with io.open('basicmoves.tex', 'w+', encoding='utf-8') as f:
		f.write(output)	
